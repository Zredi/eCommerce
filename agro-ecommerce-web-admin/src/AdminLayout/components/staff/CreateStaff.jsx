...
import { adminUpdateStaff, adminUpdateUser, createStaff, createUser } from "../../../features/userReducer";
...
function CreateStaff() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(selectedUser){
        await dispatch(adminUpdateStaff({id:selectedUser.id, userData:formData})).unwrap();
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl mt-24 mb-20 mx-auto p-6 bg-white border rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6">{selectedUser ? "Edit Staff": "Create New Staff"}</h2>
      ...
      <div className="mb-4">
        ...
        <select
          ...
          value={formData.role}
          onChange={handleChange}
          className={`shadow appearance-none border ${errors.role ? 'border-red-500' : ''
            } rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
        >
